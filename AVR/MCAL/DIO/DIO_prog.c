...
uint8 DIO_u8SetPinValue(uint8 Copy_u8Port,uint8 Copy_u8Pin,uint8 Copy_u8Value){
    uint8 Local_u8ErrorState=OK;
    if(Copy_u8Pin<=7){
   if(Copy_u8Value==DIO_u8PINVALUE_HIGH){
	   switch(Copy_u8Port){
   ...
   else if(Copy_u8Value==DIO_u8PINVALUE_LOW){
	   switch(Copy_u8Port){
...
uint8 DIO_u8ReadPinValue(uint8 Copy_u8Port,uint8 Copy_u8Pin,uint8* Copy_pu8Value){
	uint8 Local_u8ErrorState =OK;
	if(Copy_u8Pin<=7){
	if(Copy_pu8Value!=NULL){ //null
		if(Copy_u8Port==DIO_u8PORTA){
		 *Copy_pu8Value=GET_BIT(PINA,Copy_u8Pin);
		}
		else if(Copy_u8Port==DIO_u8PORTB){
			*Copy_pu8Value=GET_BIT(PINB,Copy_u8Pin);
		}
		else if(Copy_u8Port==DIO_u8PORTC){
			*Copy_pu8Value=GET_BIT(PINC,Copy_u8Pin);
				}
		else if(Copy_u8Port==DIO_u8PORTD){
